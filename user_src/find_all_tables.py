...
class ExcelPreProcessor:
    ...
    def run(self):
        ...
        user_prompt = self.user_prompt(self.txt_in_csv)
    ...
    provide me the data in each table and subtable in JSON format. I want you to extract the column headers,
    row names, and last row of row values in each table and subtable separately.
    ...
    "Table 4": {{
    "Columns": ["Gross Revenue", "Net Spending", "Net Profit"],
    "Row Names: ["Tech", "Sports", "Arts", "Entertainment"],
    "Last Row": []
    }}
    }}
    ...
    Try to add at least one type of string value to the last row (only if it exists).

    Please provide the Row Names when possible. When providing the row names, do not include the last row values as part
...
"""
    ...
    def user_prompt(self, csv_text):
        ...
        The csv text generally contains the true values present in the spreadsheet.
